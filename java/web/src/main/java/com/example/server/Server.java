...
public class Server extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse resp)
    ...
    try {
      conn = conn = DriverManager.getConnection(DB_URL, "user", "");
      Statement s = conn.createStatement();
      s.execute(
          "SELECT userName, isWin FROM users WHERE uid = " + req.getParameter("ticket") + ";");
      ...
      if (r.getBoolean("isWin") && b) {
        resp.getWriter()
            .print(
                "You win, "
                    ...
                    + "!<br>You can fill your details in with this link: "
                    + req.getParameter(id));
        ...
        if (offerResp.body() != null) {
          resp.getWriter()
              .print(
                  "An additional offer is available only for you!" + req.getParameter("offerId"));
        }
