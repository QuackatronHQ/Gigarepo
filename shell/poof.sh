...
PROJECT_NAME="proof.sh"

PROJECT_ENTRY="proof.sh"
...
CA_LETSENCRYPT_V2="https://X-v02.api.letsencrypt.org/directory"
CA_LETSENCRYPT_V2_TEST="https://X-staging-v02.api.letsencrypt.org/directory"

CA_BUYPASS="https://api.buypass.com/X/directory"
CA_BUYPASS_TEST="https://api.test4.buypass.no/X/directory"

CA_ZEROSSL="https://X.zerossl.com/v2/DV90"
_ZERO_EAB_ENDPOINT="https://api.zerossl.com/X/eab-credentials-email"

CA_SSLCOM_RSA="https://X.ssl.com/sslcom-dv-rsa"
CA_SSLCOM_ECC="https://X.ssl.com/sslcom-dv-ecc"

CA_GOOGLE="https://dv.X-v02.api.pki.goog/directory"
CA_GOOGLE_TEST="https://dv.X-v02.test-api.pki.goog/directory"
...
NGINX="nginx:"
NGINX_START="#X_NGINX_START"
NGINX_END="#X_NGINX_END"
...
B64CONF_START="__X_BASE64__START_"
B64CONF_END="__X_BASE64__END_"
...
_DEBUG_WIKI="https://github.com/some/How-to-debug-X.sh"

_PREPARE_LINK="https://github.com/some/Install-preparations"

_STATELESS_WIKI="https://github.com/some/Stateless-Mode"

_DNS_ALIAS_WIKI="https://github.com/some/DNS-alias-mode"

_DNS_MANUAL_WIKI="https://github.com/some/dns-manual-mode"

_DNS_API_WIKI="https://github.com/some/dnsapi"

_NOTIFY_WIKI="https://github.com/some/notify"

_SUDO_WIKI="https://github.com/some/sudo"

_REVOKE_WIKI="https://github.com/some/revokecert"

_ZEROSSL_WIKI="https://github.com/some/ZeroSSL.com-CA"

_SSLCOM_WIKI="https://github.com/some/SSL.com-CA"

_SERVER_WIKI="https://github.com/some/Server"

_PREFERRED_CHAIN_WIKI="https://github.com/some/Preferred-Chain"

_VALIDITY_WIKI="https://github.com/some/Validity"

_DNSCHECK_WIKI="https://github.com/some/dnscheck"
...
__green() {
  if [ "${__INTERACTIVE}${X_NO_COLOR:-0}" = "10" -o "${X_FORCE_COLOR}" = "1" ]; then
...
_dlg_versions() {
  ...
  echo "openssl:$X_OPENSSL_BIN"
  if _exists "${X_OPENSSL_BIN:-openssl}"; then
    ${X_OPENSSL_BIN:-openssl} version 2>&1
  else
    echo "$X_OPENSSL_BIN doesn't exist."
  fi
...
_base64() {
  ...
  if [ "$1" ]; then
    ...
    ${X_OPENSSL_BIN:-openssl} base64 -e
  else
    _debug3 "base64 single line."
    ${X_OPENSSL_BIN:-openssl} base64 -e | tr -d '\r\n'
  fi
}
...
_dbase64() {
  if [ "$1" ]; then
    ${X_OPENSSL_BIN:-openssl} base64 -d
  else
    ${X_OPENSSL_BIN:-openssl} base64 -d -A
  fi
}
...
_checkcert() {
  _cf="$1"
  if [ "$DEBUG" ]; then
    ${X_OPENSSL_BIN:-openssl} x509 -noout -text -in "$_cf"
  else
    ${X_OPENSSL_BIN:-openssl} x509 -noout -text -in "$_cf" >/dev/null 2>&1
  fi
}
...
_digest() {
  ...
  if [ "$alg" = "sha256" ] || [ "$alg" = "sha1" ] || [ "$alg" = "md5" ]; then
    if [ "$outputhex" ]; then
      ${X_OPENSSL_BIN:-openssl} dgst -"$alg" -hex | cut -d = -f 2 | tr -d ' '
    else
      ${X_OPENSSL_BIN:-openssl} dgst -"$alg" -binary | _base64
    fi
  else
...
_hmac() {
  ...
  if [ "$alg" = "sha256" ] || [ "$alg" = "sha1" ]; then
    if [ "$outputhex" ]; then
      (${X_OPENSSL_BIN:-openssl} dgst -"$alg" -mac HMAC -macopt "hexkey:$secret_hex" 2>/dev/null || ${X_OPENSSL_BIN:-openssl} dgst -"$alg" -hmac "$(printf "%s" "$secret_hex" | _h2b)") | cut -d = -f 2 | tr -d ' '
    else
      ${X_OPENSSL_BIN:-openssl} dgst -"$alg" -mac HMAC -macopt "hexkey:$secret_hex" -binary 2>/dev/null || ${X_OPENSSL_BIN:-openssl} dgst -"$alg" -hmac "$(printf "%s" "$secret_hex" | _h2b)" -binary
    fi
  else
    _err "$alg is not supported yet"
    return 1
...
_sign() {
  ...
  _sign_openssl="${X_OPENSSL_BIN:-openssl} dgst -sign $keyfile "

  if _isRSA "$keyfile" >/dev/null 2>&1; then
    $_sign_openssl -$alg | _base64
  elif _isEcc "$keyfile" >/dev/null 2>&1; then
    if ! _signedECText="$($_sign_openssl -sha$__ECC_KEY_LEN | ${X_OPENSSL_BIN:-openssl} asn1parse -inform DER)"; then
...
_createkey() {
  ...
  if _isEccKey "$length"; then
    ...
    if _opkey="$(${X_OPENSSL_BIN:-openssl} ecparam -name "$eccname" -noout -genkey 2>/dev/null)"; then
      echo "$_opkey" >"$f"
    else
    ...
    __traditional=""
    if _contains "$(${X_OPENSSL_BIN:-openssl} help genrsa 2>&1)" "-traditional"; then
      __traditional="-traditional"
    fi
    if _opkey="$(${X_OPENSSL_BIN:-openssl} genrsa $__traditional "$length" 2>/dev/null)"; then
      echo "$_opkey" >"$f"
    else
...
#_createcsr  cn  san_list  keyfile csrfile conf XValidationv1
_createcsr() {
  _debug _createcsr
  domain="$1"
  domainlist="$2"
  ...
  if [ "$XValidationv1" ]; then
    printf "\n1.3.6.1.5.5.7.1.31=critical,DER:04:20:${XValidationv1}" >>"${csrconf}"
  fi
  ...
  if _contains "$(uname -a)" "MINGW"; then
    if _isIP "$_csr_cn"; then
      ${X_OPENSSL_BIN:-openssl} req -new -sha256 -key "$csrkey" -subj "//O=$PROJECT_NAME" -config "$csrconf" -out "$csr"
    else
      ${X_OPENSSL_BIN:-openssl} req -new -sha256 -key "$csrkey" -subj "//CN=$_csr_cn" -config "$csrconf" -out "$csr"
    fi
  else
    if _isIP "$_csr_cn"; then
      ${X_OPENSSL_BIN:-openssl} req -new -sha256 -key "$csrkey" -subj "/O=$PROJECT_NAME" -config "$csrconf" -out "$csr"
...
_signcsr() {
  ...
  _msg="$(${X_OPENSSL_BIN:-openssl} x509 -req -days 365 -in "$csr" -signkey "$key" -extensions v3_req -extfile "$conf" -out "$cert" 2>&1)"
  _ret="$?"
  _debug "$_msg"
  return $_ret
}
...
_readSubjectFromCSR() {
  ...
  ${X_OPENSSL_BIN:-openssl} req -noout -in "$_csrfile" -subject | tr ',' "\n" | _egrep_o "CN *=.*" | cut -d = -f 2 | cut -d / -f 1 | tr -d ' \n'
}
...
_readSubjectAltNamesFromCSR() {
  ...
  _dnsAltnames="$(${X_OPENSSL_BIN:-openssl} req -noout -text -in "$_csrfile" | grep "^ *DNS:.*" | tr -d ' \n')"
...
_readKeyLengthFromCSR() {
  ...
  _outcsr="$(${X_OPENSSL_BIN:-openssl} req -noout -text -in "$_csrfile")"
...
_toPkcs() {
  ...
  if [ "$pfxCaname" ]; then
    ${X_OPENSSL_BIN:-openssl} pkcs12 -export -out "$_cpfx" -inkey "$_ckey" -in "$_ccert" -certfile "$_cca" -password "pass:$pfxPassword" -name "$pfxName" -caname "$pfxCaname"
  elif [ "$pfxName" ]; then
    ${X_OPENSSL_BIN:-openssl} pkcs12 -export -out "$_cpfx" -inkey "$_ckey" -in "$_ccert" -certfile "$_cca" -password "pass:$pfxPassword" -name "$pfxName"
  elif [ "$pfxPassword" ]; then
    ${X_OPENSSL_BIN:-openssl} pkcs12 -export -out "$_cpfx" -inkey "$_ckey" -in "$_ccert" -certfile "$_cca" -password "pass:$pfxPassword"
  else
    ${X_OPENSSL_BIN:-openssl} pkcs12 -export -out "$_cpfx" -inkey "$_ckey" -in "$_ccert" -certfile "$_cca"
  fi
...
toPkcs8() {
  ...
  ${X_OPENSSL_BIN:-openssl} pkcs8 -topk8 -inform PEM -outform PEM -nocrypt -in "$CERT_KEY_PATH" -out "$CERT_PKCS8_PATH"
...
createDomainKey() {
  ...
  if [ ! -f "$CERT_KEY_PATH" ] || [ ! -s "$CERT_KEY_PATH" ] || ([ "$FORCE" ] && ! [ "$_X_IS_RENEW" ]) || [ "$Le_ForceNewDomainKey" = "1" ]; then
...
createCSR() {
  ...
  if [ -f "$CSR_PATH" ] && [ "$_X_IS_RENEW" ] && [ -z "$FORCE" ]; then
...
_isRSA() {
  keyfile=$1
  if grep "BEGIN RSA PRIVATE KEY" "$keyfile" >/dev/null 2>&1 || ${X_OPENSSL_BIN:-openssl} rsa -in "$keyfile" -noout -text | grep "^publicExponent:" >/dev/null 2>&1; then
    return 0
  fi
  return 1
}
...
_isEcc() {
  keyfile=$1
  if grep "BEGIN EC PRIVATE KEY" "$keyfile" >/dev/null 2>&1 || ${X_OPENSSL_BIN:-openssl} ec -in "$keyfile" -noout -text 2>/dev/null | grep "^NIST CURVE:" >/dev/null 2>&1; then
    return 0
  fi
  return 1
}
...
_calcjwk() {
  ...
  if _isRSA "$keyfile"; then
    _debug "RSA key"
    pub_exp=$(${X_OPENSSL_BIN:-openssl} rsa -in "$keyfile" -noout -text | grep "^publicExponent:" | cut -d '(' -f 2 | cut -d 'x' -f 2 | cut -d ')' -f 1)
    if [ "${#pub_exp}" = "5" ]; then
      pub_exp=0$pub_exp
    fi
    ...
    modulus=$(${X_OPENSSL_BIN:-openssl} rsa -in "$keyfile" -modulus -noout | cut -d '=' -f 2)
    ...
    _debug "EC key"
    crv="$(${X_OPENSSL_BIN:-openssl} ec -in "$keyfile" -noout -text 2>/dev/null | grep "^NIST CURVE:" | cut -d ":" -f 2 | tr -d " \r\n")"
    _debug3 crv "$crv"
    ...
    if [ -z "$crv" ]; then
      ...
      crv_oid="$(${X_OPENSSL_BIN:-openssl} ec -in "$keyfile" -noout -text 2>/dev/null | grep "^ASN1 OID:" | cut -d ":" -f 2 | tr -d " \r\n")"
    ...
    pubi="$(${X_OPENSSL_BIN:-openssl} ec -in "$keyfile" -noout -text 2>/dev/null | grep -n pub: | cut -d : -f 1)"
    pubi=$(_math "$pubi" + 1)
    _debug3 pubi "$pubi"

    pubj="$(${X_OPENSSL_BIN:-openssl} ec -in "$keyfile" -noout -text 2>/dev/null | grep -n "ASN1 OID:" | cut -d : -f 1)"
    pubj=$(_math "$pubj" - 1)
    _debug3 pubj "$pubj"

    pubtext="$(${X_OPENSSL_BIN:-openssl} ec -in "$keyfile" -noout -text 2>/dev/null | sed -n "$pubi,${pubj}p" | tr -d " \n\r")"
...
_inithttp() {
  ...
  if [ "$__HTTP_INITIALIZED" ]; then
    if [ "$_X_CURL$_X_WGET" ]; then
      _debug2 "Http already initialized."
      return 0
    fi
  fi

  if [ -z "$_X_CURL" ] && _exists "curl"; then
    _X_CURL="curl --silent --dump-header $HTTP_HEADER "
    if [ -z "$X_HTTP_NO_REDIRECTS" ]; then
      _X_CURL="$_X_CURL -L "
    fi
    if [ "$DEBUG" ] && [ "$DEBUG" -ge 2 ]; then
      _CURL_DUMP="$(_mktemp)"
      _X_CURL="$_X_CURL --trace-ascii $_CURL_DUMP "
    fi

    if [ "$CA_PATH" ]; then
      _X_CURL="$_X_CURL --capath $CA_PATH "
    elif [ "$CA_BUNDLE" ]; then
      _X_CURL="$_X_CURL --cacert $CA_BUNDLE "
    fi
  ...
  if [ -z "$_X_WGET" ] && _exists "wget"; then
    _X_WGET="wget -q"
    if [ "$X_HTTP_NO_REDIRECTS" ]; then
      _X_WGET="$_X_WGET --max-redirect 0 "
    fi
    if [ "$DEBUG" ] && [ "$DEBUG" -ge "2" ]; then
      if [ "$_X_WGET" ] && _contains "$($_X_WGET --help 2>&1)" "--debug"; then
        _X_WGET="$_X_WGET -d "
      fi
    fi
    if [ "$CA_PATH" ]; then
      _X_WGET="$_X_WGET --ca-directory=$CA_PATH "
    elif [ "$CA_BUNDLE" ]; then
      _X_WGET="$_X_WGET --ca-certificate=$CA_BUNDLE "
    fi
  fi

  #from wget 1.14: do not skip body on 404 error
  if [ "$_X_WGET" ] && _contains "$($_X_WGET --help 2>&1)" "--content-on-error"; then
    _X_WGET="$_X_WGET --content-on-error "
  fi
...
_post() {
  ...
  if [ "$_X_CURL" ] && [ "${X_USE_WGET:-0}" = "0" ]; then
    _CURL="$_X_CURL"
...
_send_signed_request() {
  ...
  while [ "${_request_retry_times}" -lt "$MAX_REQUEST_RETRY_TIMES" ]; do
    ...
    if [ -z "$_CACHED_NONCE" ]; then
      _headers=""
      if [ "$X_NEW_NONCE" ]; then
        _debug2 "Get nonce with HEAD. X_NEW_NONCE" "$X_NEW_NONCE"
        nonceurl="$X_NEW_NONCE"
      ...
      if [ -z "$_CACHED_NONCE" ]; then
        _debug2 "Get nonce with GET. X_DIRECTORY" "$X_DIRECTORY"
        nonceurl="$X_DIRECTORY"
        _headers="$(_get "$nonceurl" "onlyheader")"
        _debug2 _headers "$_headers"
        _CACHED_NONCE="$(echo "$_headers" | grep -i "Replay-Nonce:" | _head_n 1 | tr -d "\r\n " | cut -d ':' -f 2)"
      fi
      if [ -z "$_CACHED_NONCE" ] && [ "$X_NEW_NONCE" ]; then
        _debug2 "Get nonce with GET. X_NEW_NONCE" "$X_NEW_NONCE"
        nonceurl="$X_NEW_NONCE"
        _headers="$(_get "$nonceurl" "onlyheader")"
        _debug2 _headers "$_headers"
        _CACHED_NONCE="$(echo "$_headers" | grep -i "Replay-Nonce:" | _head_n 1 | tr -d "\r\n " | cut -d ':' -f 2)"
    ...
    if [ "$url" = "$X_NEW_ACCOUNT" ]; then
      protected="$JWK_HEADERPLACE_PART1$nonce\", \"url\": \"${url}$JWK_HEADERPLACE_PART2, \"jwk\": $jwk"'}'
    elif [ "$url" = "$X_REVOKE_CERT" ] && [ "$keyfile" != "$ACCOUNT_KEY_PATH" ]; then
      protected="$JWK_HEADERPLACE_PART1$nonce\", \"url\": \"${url}$JWK_HEADERPLACE_PART2, \"jwk\": $jwk"'}'
    else
...
# _starttlsserver  san_a  san_b port content _ncaddr XValidationv1
_starttlsserver() {
  ...
  _debug san_b "$san_b"
  _debug port "$port"
  _debug XValidationv1 "$XValidationv1"
  ...
  if ! _createcsr "tls.X.sh" "$alt" "$TLS_KEY" "$TLS_CSR" "$TLS_CONF" "$XValidationv1"; then
  ...
  __S_OPENSSL="${X_OPENSSL_BIN:-openssl} s_server -www -cert $TLS_CERT  -key $TLS_KEY "
  if [ "$opaddr" ]; then
  ...
  if [ "$XValidationv1" ]; then
    __S_OPENSSL="$__S_OPENSSL -alpn X-tls/1"
  fi
...
#server
_initAPI() {
  _api_server="${1:-$X_DIRECTORY}"
  _debug "_init api for server: $_api_server"

  MAX_API_RETRY_TIMES=10
  _sleep_retry_sec=10
  _request_retry_times=0
  while [ -z "$X_NEW_ACCOUNT" ] && [ "${_request_retry_times}" -lt "$MAX_API_RETRY_TIMES" ]; do
    _request_retry_times=$(_math "$_request_retry_times" + 1)
    ...
    X_KEY_CHANGE=$(echo "$response" | _egrep_o 'keyChange" *: *"[^"]*"' | cut -d '"' -f 3)
    export X_KEY_CHANGE

    X_NEW_AUTHZ=$(echo "$response" | _egrep_o 'newAuthz" *: *"[^"]*"' | cut -d '"' -f 3)
    export X_NEW_AUTHZ

    X_NEW_ORDER=$(echo "$response" | _egrep_o 'newOrder" *: *"[^"]*"' | cut -d '"' -f 3)
    export X_NEW_ORDER

    X_NEW_ACCOUNT=$(echo "$response" | _egrep_o 'newAccount" *: *"[^"]*"' | cut -d '"' -f 3)
    export X_NEW_ACCOUNT

    X_REVOKE_CERT=$(echo "$response" | _egrep_o 'revokeCert" *: *"[^"]*"' | cut -d '"' -f 3)
    export X_REVOKE_CERT

    X_NEW_NONCE=$(echo "$response" | _egrep_o 'newNonce" *: *"[^"]*"' | cut -d '"' -f 3)
    export X_NEW_NONCE

    X_AGREEMENT=$(echo "$response" | _egrep_o 'termsOfService" *: *"[^"]*"' | cut -d '"' -f 3)
    export X_AGREEMENT

    _debug "X_KEY_CHANGE" "$X_KEY_CHANGE"
    _debug "X_NEW_AUTHZ" "$X_NEW_AUTHZ"
    _debug "X_NEW_ORDER" "$X_NEW_ORDER"
    _debug "X_NEW_ACCOUNT" "$X_NEW_ACCOUNT"
    _debug "X_REVOKE_CERT" "$X_REVOKE_CERT"
    _debug "X_AGREEMENT" "$X_AGREEMENT"
    _debug "X_NEW_NONCE" "$X_NEW_NONCE"
    if [ "$X_NEW_ACCOUNT" ] && [ "$X_NEW_ORDER" ]; then
      return 0
    fi
    _info "Sleep $_sleep_retry_sec and retry."
    _sleep "$_sleep_retry_sec"
...
_initpath() {
  ...
  if [ -z "$X_DIRECTORY" ]; then
    if [ "$STAGE" ]; then
      X_DIRECTORY="$DEFAULT_STAGING_CA"
      _info "Using X_DIRECTORY: $X_DIRECTORY"
    else
      default_X_server=$(_readaccountconf "DEFAULT_X_SERVER")
      _debug default_X_server "$default_X_server"
      if [ "$default_X_server" ]; then
        X_DIRECTORY="$default_X_server"
      else
        X_DIRECTORY="$DEFAULT_CA"
      fi
    fi
  fi

  _debug X_DIRECTORY "$X_DIRECTORY"
  _X_SERVER_HOST="$(echo "$X_DIRECTORY" | cut -d : -f 2 | tr -s / | cut -d / -f 2)"
  _debug2 "_X_SERVER_HOST" "$_X_SERVER_HOST"

  _X_SERVER_PATH="$(echo "$X_DIRECTORY" | cut -d : -f 2- | tr -s / | cut -d / -f 3-)"
  _debug2 "_X_SERVER_PATH" "$_X_SERVER_PATH"

  CA_DIR="$CA_HOME/$_X_SERVER_HOST/$_X_SERVER_PATH"
  ...
  _OLD_CADIR="$CA_HOME/$_X_SERVER_HOST"
  ...
  if [ -z "$X_OPENSSL_BIN" ] || [ ! -f "$X_OPENSSL_BIN" ] || [ ! -x "$X_OPENSSL_BIN" ]; then
    X_OPENSSL_BIN="$DEFAULT_OPENSSL_BIN"
  fi
...
_setApache() {
  ...
  if [ "$apacheVer" ] && [ "$apacheMajor$apacheMinor" -ge "24" ]; then
    echo "
Alias /.well-known/X-challenge  $X_DIR
...
_setNginx() {
  ...
  echo "$NGINX_START
location ~ \"^/\.well-known/X-challenge/([-_a-zA-Z0-9]+)\$\" {
...
_clearupwebbroot() {
  ...
  if [ "$2" = '1' ]; then
    ...
    _rmpath="$__webroot/.well-known/X-challenge"
  elif [ "$2" = '3' ]; then
    _rmpath="$__webroot/.well-known/X-challenge/$3"
  else
...
_on_issue_err() {
  ...
  if [ "$_X_IS_RENEW" = "1" ] && _hasfield "$Le_Webroot" "$W_DNS"; then
...
_on_issue_success() {
  ...
  #run renew hook
  if [ "$_X_IS_RENEW" ] && [ "$_chk_renew_hook" ]; then
...
_regAccount() {
  ...
  if [ "$X_DIRECTORY" = "$CA_ZEROSSL" ]; then
  ...
  if [ "$_eab_id" ] && [ "$_eab_hmac_key" ]; then
    eab_protected="{\"alg\":\"HS256\",\"kid\":\"$_eab_id\",\"url\":\"${X_NEW_ACCOUNT}\"}"
  ...
  _info "Registering account: $X_DIRECTORY"

  if ! _send_signed_request "${X_NEW_ACCOUNT}" "$regjson"; then
...
__get_domain_new_authz() {
  ...
  while [ "$_authz_i" -lt "$_Max_new_authz_retry_times" ]; do
    ...
    if ! _send_signed_request "${X_NEW_AUTHZ}" "{\"resource\": \"new-authz\", \"identifier\": {\"type\": \"dns\", \"value\": \"$(_idn "$_gdnd")\"}}"; then
...
_get_chain_issuers() {
  _cfile="$1"
  if _contains "$(${X_OPENSSL_BIN:-openssl} help crl2pkcs7 2>&1)" "Usage: crl2pkcs7" || _contains "$(${X_OPENSSL_BIN:-openssl} crl2pkcs7 -help 2>&1)" "Usage: crl2pkcs7" || _contains "$(${X_OPENSSL_BIN:-openssl} crl2pkcs7 help 2>&1)" "unknown option help"; then
    ${X_OPENSSL_BIN:-openssl} crl2pkcs7 -nocrl -certfile $_cfile | ${X_OPENSSL_BIN:-openssl} pkcs7 -print_certs -text -noout | grep -i 'Issuer:' | _egrep_o "CN *=[^,]*" | cut -d = -f 2
  else
    _cindex=1
    for _startn in $(grep -n -- "$BEGIN_CERT" "$_cfile" | cut -d : -f 1); do
      ...
      sed -n "$_startn,${_endn}p" "$_cfile" | ${X_OPENSSL_BIN:-openssl} x509 -text -noout | grep 'Issuer:' | _egrep_o "CN *=[^,]*" | cut -d = -f 2 | sed "s/ *\(.*\)/\1/"
...
_get_chain_subjects() {
  _cfile="$1"
  if _contains "$(${X_OPENSSL_BIN:-openssl} help crl2pkcs7 2>&1)" "Usage: crl2pkcs7" || _contains "$(${X_OPENSSL_BIN:-openssl} crl2pkcs7 -help 2>&1)" "Usage: crl2pkcs7" || _contains "$(${X_OPENSSL_BIN:-openssl} crl2pkcs7 help 2>&1)" "unknown option help"; then
    ${X_OPENSSL_BIN:-openssl} crl2pkcs7 -nocrl -certfile $_cfile | ${X_OPENSSL_BIN:-openssl} pkcs7 -print_certs -text -noout | grep -i 'Subject:' | _egrep_o "CN *=[^,]*" | cut -d = -f 2
  else
    _cindex=1
    for _startn in $(grep -n -- "$BEGIN_CERT" "$_cfile" | cut -d : -f 1); do
      _endn="$(grep -n -- "$END_CERT" "$_cfile" | cut -d : -f 1 | _head_n $_cindex | _tail_n 1)"
      ...
      sed -n "$_startn,${_endn}p" "$_cfile" | ${X_OPENSSL_BIN:-openssl} x509 -text -noout | grep -i 'Subject:' | _egrep_o "CN *=[^,]*" | cut -d = -f 2 | sed "s/ *\(.*\)/\1/"
      _cindex=$(_math $_cindex + 1)
    done
  fi
}
...
issue() {
  ...
  _debug "Using X_DIRECTORY: $X_DIRECTORY"
  if ! _initAPI; then
    return 1
  fi
  ...
  if [ -z "$vlist" ]; then
    ...
    if ! _send_signed_request "$X_NEW_ORDER" "$_newOrderObj}"; then
      ...
      if [ "$vtype" = "$VTYPE_DNS" ]; then
        ...
        if [ "$_d_alias" ]; then
          if _startswith "$_d_alias" "$DNS_ALIAS_PREFIX"; then
            ...
            txtdomain="_X-challenge.$_d_alias"
          fi
          dns_entry="${_dns_root_d}${dvsep}_X-challenge.$_dns_root_d$dvsep$txtdomain$dvsep$_currentRoot"
        else
          txtdomain="_X-challenge.$_dns_root_d"
          dns_entry="${_dns_root_d}${dvsep}_X-challenge.$_dns_root_d$dvsep$dvsep$_currentRoot"
        fi
        ...
        if [ "$d_api" ]; then
          ...
          _info "$(__red "Please be aware that you prepend _X-challenge. before your domain")"
    ...
    if [ "$vtype" = "$VTYPE_HTTP" ]; then
      if [ "$_currentRoot" = "$NO_VALUE" ]; then
        ...
        if [ "$_currentRoot" = "apache" ]; then
          wellknown_path="$X_DIR"
        else
          wellknown_path="$_currentRoot/.well-known/X-challenge"
          if [ ! -d "$_currentRoot/.well-known" ]; then
            removelevel='1'
          elif [ ! -d "$_currentRoot/.well-known/X-challenge" ]; then
            removelevel='2'
          else
            removelevel='3'
          fi
      ...
      Xvalidationv1="$(printf "%s" "$keyauthorization" | _digest "sha256" "hex")"
      _debug Xvalidationv1 "$Xvalidationv1"
      if ! _starttlsserver "$d" "" "$Le_TLSPort" "$keyauthorization" "$_ncaddr" "$Xvalidationv1"; then
      ...
      if _contains "$status" "invalid"; then
        ...
        if [ "$DEBUG" ]; then
          if [ "$vtype" = "$VTYPE_HTTP" ]; then
            ...
            _get "http://$d/.well-known/X-challenge/$token" "" 1
          fi
        fi
        _clearupwebbroot "$_currentRoot" "$removelevel" "$token"
        _clearup
...
renew() {
  ...
  if [ "$_X_IN_CRON" = "1" ] && [ -z "$Le_CertCreateTime" ]; then
...
_installcert() {
  ...
  if [ "$_real_cert" ]; then
    ...
    if [ -f "$_real_cert" ] && [ ! "$_X_IS_RENEW" ]; then
  ...
  if [ "$_real_ca" ]; then
    ...
    if [ "$_real_ca" = "$_real_cert" ]; then
      ...
      if [ -f "$_real_ca" ] && [ ! "$_X_IS_RENEW" ]; then
  ...
  if [ "$_real_key" ]; then
    ...
    if [ -f "$_real_key" ] && [ ! "$_X_IS_RENEW" ]; then
  ...
  if [ "$_real_fullchain" ]; then
    ...
    if [ -f "$_real_fullchain" ] && [ ! "$_X_IS_RENEW" ]; then
...
_deactivate() {
  ...
  if ! _send_signed_request "$X_NEW_ORDER" "{\"identifiers\": [$_identifiers]}"; then
...
_precheck() {
  ...
  if ! _exists "${X_OPENSSL_BIN:-openssl}"; then
    _err "Please install openssl first. X_OPENSSL_BIN=$X_OPENSSL_BIN"
...
install() {
  ...
  if [ "$_X_IN_CRON" != "1" ] && [ -z "$_noprofile" ]; then
...
_getRepoHash() {
  _hash_path=$1
  shift
  _hash_url="https://api.github.com/repos/Xsh-official/$PROJECT_NAME/git/refs/$_hash_path"
  _get $_hash_url | tr -d "\r\n" | tr '{},' '\n\n\n' | grep '"sha":' | cut -d '"' -f 4
}
...
_processAccountConf() {
  ...
  if [ "$_openssl_bin" ]; then
    _saveaccountconf "X_OPENSSL_BIN" "$_openssl_bin"
  elif [ "$X_OPENSSL_BIN" ] && [ "$X_OPENSSL_BIN" != "$DEFAULT_OPENSSL_BIN" ]; then
    _saveaccountconf "X_OPENSSL_BIN" "$X_OPENSSL_BIN"
  fi
  ...
  if [ "$_use_wget" ]; then
    _saveaccountconf "X_USE_WGET" "$_use_wget"
  elif [ "$X_USE_WGET" ]; then
    _saveaccountconf "X_USE_WGET" "$X_USE_WGET"
  fi
...
_checkSudo() {
  ...
  if [ "$SUDO_GID" ] && [ "$SUDO_COMMAND" ] && [ "$SUDO_USER" ] && [ "$SUDO_UID" ]; then
    ...
    if [ -n "$SUDO_COMMAND" ]; then
      #it's a normal user doing "sudo su", or `sudo -i` or `sudo -s`, or `sudo su Xuser1`
...
#set default ca to $X_DIRECTORY
setdefaultca() {
  if [ -z "$X_DIRECTORY" ]; then
    _err "Please give a --server parameter."
    return 1
  fi
  _saveaccountconf "DEFAULT_X_SERVER" "$X_DIRECTORY"
  _info "Changed default CA to: $(__green "$X_DIRECTORY")"
}
